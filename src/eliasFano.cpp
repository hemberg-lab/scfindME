  ...
  for (int gene_row = 0; gene_row < gene_matrix.nrow(); ++gene_row)
  {
    ...
    for (Rcpp::NumericVector::const_iterator expr = expression_vector.begin(); expr != expression_vector.end(); ++expr)
    {
      ...
      if (*expr > 0)
      {
        ...
        // here is a problem, where does * expr points to? this need to be modified to suit the new ASindex function
...
Rcpp::DataFrame EliasFanoDB::findMarkerGenes(const Rcpp::CharacterVector& gene_list, const Rcpp::CharacterVector& datasets_active,unsigned int min_support_cutoff = 3, bool console_message = false)
{
