...
option_list <- list( 
    ...
    make_option(c("-p", "--pseudobulk_psi"), type = "character", default=NULL,
        help="Combined pseudobulk psi matrix as input, tab-deliminated"),
...
# temporary, hard-coded nodes info file
# will be included in final package

ni <- readRDS("/lustre/scratch117/cellgen/team218/ys6/VASA-seq/data/nodes_info.rds")
...
buildMatrix.original <- function(file, num_reads_min){
  ...
  matrix.original <- data %>% 
    ...
    dplyr::mutate(Filter = case_when(Total_Reads < num_reads_min ~ "DROP",
                                     Total_Reads >= num_reads_min ~ "KEEP",
                                     TRUE ~ NA_character_
    )) %>%
